 ...
 * Each transcript is characterized by a student’s name (e.g., "Sunhye"), 
 * 	a unique student id, a list of up to 60 current registrations.
 *  
 * Notice that each unique student ID is the concatenation of 
 * 	the student’s name, and an integer that is a positive multiple of 3, e.g., "Sunhye3".
 * 
...
public class Tests {
    ...
    @Test
    public void test_20() {
    	/* Create a transcript with the student's name (e.g., "A"). 
		 * A transcript can store up to and including 60 registrations. 
		 */
		Transcript t1 = new Transcript("Alan");
		Transcript t2 = new Transcript("Bob");
		Transcript t3 = new Transcript("Cathy");
		Transcript t4 = new Transcript("David");
		Transcript t5 = new Transcript("Emily");
		Transcript t6 = new Transcript("Fiona");
		
		/* The ID of a student is the concatenation of two parts (with no space in between): 
		 * 1) the student's name; 2) an integer counter. 
		 * The integer counter is a positive multiple of three: 3, 6, 9, 12, ...
		 * For examples, 	t1 is for the 1st student "Sunhye", the id is "Sunhye3".
		 * 				  	t2 is for the 2nd student "Jihye", the id is "Jihye6".
		 */
        assertEquals("Alan3", t1.getStudentID());
        assertEquals("Bob6", t2.getStudentID());
        assertEquals("Cathy9", t3.getStudentID());
		assertEquals("David12", t4.getStudentID());
		assertEquals("Emily15", t5.getStudentID());
		assertEquals("Fiona18", t6.getStudentID());
    }//first five transcript objects have the correct ids
	...
	public void test_48() {
		...
		/* Given a valid student ID and an existing course name of that student,
		 * return the marks for that course.
		 */
		assertEquals(50, rs.getMarks(alan.getStudentID(), "EECS1"));
		assertEquals(60, rs.getMarks(alan.getStudentID(), "EECS2"));
		assertEquals(70, rs.getMarks(alan.getStudentID(), "EECS3"));
		assertEquals(80, rs.getMarks(alan.getStudentID(), "EECS4"));
	...
	public void test_49() {
		...
		assertEquals(-1, rs.getMarks(alan.getStudentID(), "EECS5")); 
	...
	public void test_50() {
		...
		assertEquals(90, rs.getMarks(mark.getStudentID(), "EECS5"));
		assertEquals(80, rs.getMarks(mark.getStudentID(), "EECS6"));
		assertEquals(70, rs.getMarks(mark.getStudentID(), "EECS7"));
		assertEquals(60, rs.getMarks(mark.getStudentID(), "EECS8"));
	...
	public void test_51() {
		...
		/* If the student ID is valid, but the course does not exist on that student's transcript,
		 * then return -1 as the marks.
		 */
		assertEquals(-1, rs.getMarks(mark.getStudentID(), "EECS9")); 
	...
	public void test_52() {
		...
		/* If the student ID is not valid, then return -1 as the marks.
		 */
		assertEquals(-1, rs.getMarks(mark.getStudentID() + "junk", "EECS4")); 
